...
public class ThrowingEggs
{
	...
	public static int TwoEggs(int F,int N)
	{
		...
		{
			sum+=--x;
			if(sum>N)//be careful with the case when sum reaches N
				StdOut.print(N+" ");
			else
				StdOut.print(sum+" ");
			cnt++;
	...
	public static int TwoEggsFast(int F,int N)
	{
		...
		//Method same to TwoEggs, but for output, we should revise a little.
		F=F-i/2;N=i/2;//treat as a ( ( i-(i/2+1) ) + 1 )-story building, i/2+1 as the first floor
		int cnt2=1,x=(int)Math.ceil((-1+Math.sqrt(1+8*N))/2),sum=x;
		if(sum+i/2!=i)//in case of 1 2 4 -> find 4 in 3,4, but first test 4
		...
		{
			sum+=--x;
			if(sum<=N && sum+i/2!=i)//if sum>N, we will test N+i/2=i which has already been tested: e.g.: F=16
				StdOut.print(sum+i/2+" ");
			else
				cnt2--;
			cnt2++;
